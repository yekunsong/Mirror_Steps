...
public final class Level8 extends BaseLevel { 
    ...
    protected void buildLevel() {
        ...
        // MIDDLE TIER (Goal area)
        addLevelPlatform(0, 470, 169, 60);       // Left ledge
        addLevelPlatform(435, 470, 300, 60);     // Center island (Goal here)
        addLevelPlatform(1017, 470, 153, 60);    // Right ledge

        // BOTTOM FLOOR
        addLevelPlatform(0, 604, 1170, 59);

        // GOAL: Center of the middle island
        setGoal(scaleX(560), scaleY(395));
        
        // Clear old nodes before adding new ones
        for(Angel a : angels) root.getChildren().remove(a.node);
        angels.clear();

        // ANGELS ON MIDDLE TIER )
        addAngel(scaleX(450), scaleY(425)); // Left of goal
        addAngel(scaleX(690), scaleY(425)); // Right of goal
        
        // ANGELS ON BOTTOM FLOOR 
        addAngel(scaleX(450), scaleY(560)); // Bottom Left
        addAngel(scaleX(570), scaleY(560)); // Bottom Center
        addAngel(scaleX(690), scaleY(560)); // Bottom Right
    }
    ...
    private void createPlayer() {
        // Spawn in the top center of the screen
        ...
        for (Angel angel : angels) {
            ...
            if (angel.isActive) {
                ...
                if (player.getBounds().intersects(angel.node.getBoundsInParent())) {
                    onSolidPlayerOutOfWorld(); // This now starts the red fade
                }
            }
        }
    }
